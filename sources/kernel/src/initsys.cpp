...
extern "C" void __attribute__((section(".initsys"))) __attribute__((noreturn)) _init_system_memory()
{
    ...
    // tady rozdelujeme prostor na ten, ktery bude prekladat registr TTBR0 a TTBR1, nastavenim na 16k boundary zajistime,
    // ze se vsechny preklady budou cinit jen pomoci TTBR0 registru
    ...
    // zapneme vsechno mozne v MMUCR registru - zapneme MMU, datovou a instrukcni cache, predikci skoku a zapneme si extended stranky
    unsigned int mmucr;
     ...
     * 
     * Nutno dodat, ze kernel bude takto nahrany v KAZDEM pametovem prostoru VSECH procesu, coz dovoluje v kontextu daneho procesu
     * vykonavat napriklad obsluhy systemovych volani (prepne se tim jen privilege level, a razem muzeme kod kernelu spoustet a data
