...
CMonitor& CMonitor::operator<<(float num)
{
    /*static constexpr unsigned int BUFFER_SIZE = 16;
    static char s_buffer[BUFFER_SIZE];

    // Pøevod float na øetìzec s pevnou pøesností na dvì desetinná místa
    int int_part = (int)num;
    int decimal_part = (int)((num - int_part) * 100);
    if (decimal_part < 0) decimal_part = -decimal_part; // pro záporná èísla

    unsigned int idx = 0;

    // Ošetøení záporných hodnot
    if (num < 0) {
        s_buffer[idx++] = '-';
        int_part = -int_part;
    }
    ...
    // Pøevod celé èásti
    itoa(int_part, &s_buffer[idx], 10);

    while (s_buffer[idx] != '\0') {
        idx++;
    }

    s_buffer[idx++] = '.';

    // Pøevod desetinné èásti (vždy dvì èíslice)
    s_buffer[idx++] = '0' + (decimal_part / 10);
    s_buffer[idx++] = '0' + (decimal_part % 10);
